...
<body>
 ...
 <?php
  ...
  $consulta = mysqli_query($con, "insert into pedidos() values ()") or die(mysqli_error($con));
 ...
 <div class="container">
  <div class="row mt-4">
    <div class="col-md">
      ...
      <div class="form-group row">
        <label for="Fecha" class="col-lg-3 col-form-label">Fecha de facturación:</label>
        <div class="col-lg-3">
      ...
      <div class="form-group row">
        ...
        <div class="col-lg-3">
          <select class="form-control" id="CodigoCliente">
            <?php
            $consulta = mysqli_query($con, "select nit, empresa from clientes") or die(mysqli_error($con));
            $clientes = mysqli_fetch_all($consulta, MYSQLI_ASSOC);
            echo "<option value='0'>Seleccionar Cliente</option>";
            foreach ($clientes as $cli) {
              echo "<option value='" . $cli['nit'] . "'>" . $cli['empresa'] . "</option>";
            }
            ?>
          </select>
        </div>
      ...
      <div class="form-group row">
    <label for="CodigoEmpleado" class="col-lg-3 col-form-label">Empleado:</label>
    <div class="col-lg-3">
        <select class="form-control" id="CodigoEmpleado" disabled>
            <?php
            $consulta = mysqli_query($con, "select id, nombre from empleado") or die(mysqli_error($con));
            $empleados = mysqli_fetch_all($consulta, MYSQLI_ASSOC);
            foreach ($empleados as $emp) {
                echo "<option value='" . $emp['id'] . "'>" . $emp['nombre'] . "</option>";
            }
            ?>
        </select>
        <small class="form-text text-muted">Empleado seleccionado</small>
    </div>
</div>

      <div class="form-group row">
    <label for="Comentario" class="col-lg-3 col-form-label">Tipo de Pago:</label>
    <div class="col-lg-3">
        <input type="text" class="form-control" id="Comentario" value="Pago con tarjeta" disabled>
    </div>
</div>
      ...
      <div class="modal-body">
        <div class="form-group">
          ...
          <select class="form-control" id="CodigoProducto">
            <?php
            $consulta = mysqli_query($con, "select codigo, nombre, valorunitario from productos") or die(mysqli_error($con));
            $productos = mysqli_fetch_all($consulta, MYSQLI_ASSOC);
            foreach ($productos as $pro) {
              echo "<option value='" . $pro['codigo'] . "'>" . $pro['nombre'] . '  ($' . $pro['valorunitario'] . ")</option>";
            }
            ?>
          </select>
        </div>
...
<script>
 ...
 $('#btnConfirmarFactura').click(function() {
  ...
  function EnviarInformacionFactura(accion) {
    ...
    data: cliente, 
    success: function(msg) {
       window.location = '../Menu/MenuClientes.php';
    },
    error: function() {
     window.location = '../Menu/MenuClientes.php';
    }
  ...
  function EnviarInformacionFacturaImprimir(accion) {
    ...
    data: cliente,
    success: function(msg) {
      window.open('PDF_Factura.php?' + '&codigofactura=' + <?php echo $codigofactura ?>, '_blank');
      window.location = '../Menu/MenuClientes.php';
    },
    error: function() {
     alert("Hay un error ..");
